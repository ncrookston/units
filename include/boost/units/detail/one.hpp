...
BOOST_CONSTEXPR inline one make_one() {
    one result;
    return(result);
}
...
template<class T>
BOOST_CONSTEXPR inline T operator*(const one&, const T& t)
{
    return(t);
}

template<class T>
BOOST_CONSTEXPR inline T operator*(const T& t, const one&)
{
    return(t);
}

BOOST_CONSTEXPR inline one operator*(const one&, const one&)
{
    one result;
    return(result);
}
...
template<class T>
BOOST_CONSTEXPR inline bool operator>(const boost::units::one&, const T& t) {
    return(1 > t);
}
...
BOOST_CONSTEXPR inline double one_to_double(const one&) { return 1.0; }
