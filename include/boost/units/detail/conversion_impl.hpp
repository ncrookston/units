...
struct base_unit_converter_base<Source, BOOST_UNITS_MAKE_HETEROGENEOUS_UNIT(Source, typename Source::dimension_type)>
{
    ...
    typedef one type;
    BOOST_CONSTEXPR static type value() {
        one result;
        return(result);
    }
};
...
struct get_default_conversion_impl<0>
{
    ...
    struct apply
    {
        ...
        typedef one type;
        BOOST_CONSTEXPR static one value() {
            one result;
            return(result);
        }
    };
...
struct conversion_impl
{
    ...
    struct apply
    {
        ...
        BOOST_CONSTEXPR static type value() { return(static_rational_power<typename unit_pair::value_type>(converter::value()) * next_iteration::value()); }
    };
};
...
struct conversion_impl<0>
{
    ...
    {
        typedef one type;
        BOOST_CONSTEXPR static type value() { one result; return(result); }
    };
};
...
struct conversion_helper<quantity<Unit1, T1>, quantity<Unit2, T2> >
{
    ...
    BOOST_CONSTEXPR static destination_type convert(const quantity<Unit1, T1>& source)
    {
        Unit1 u1;
        Unit2 u2;
